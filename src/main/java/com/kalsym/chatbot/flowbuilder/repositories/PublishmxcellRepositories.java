...
import com.kalsym.chatbot.flowbuilder.mxmodel.daos.Publishmxcell;
 ...
 * @author user
 */
public interface PublishmxcellRepositories extends MongoRepository<Publishmxcell, String> {
     ...
     */
   @Query("{'flowId' : ?0}")
    public List <Publishmxcell> getMxByFlowId(
    ...
    public List <Publishmxcell> deleteMxcellByFlowId(String flowId);
   ...
   @Query("{'flowId' : ?0, 'mxId' : ?1}")
    public Publishmxcell findByMxId(
            @Param("flowId") String flowId,
   ...
   @Query("{'flowId' : ?0, $or: [ {'mxCell.@source':?1},{'mxCell.@target':?1}]}")
    public List<Publishmxcell> findByUserObjectMxId(
            @Param("flowId") String flowId,
