...
import com.kalsym.chatbot.flowbuilder.mxmodel.daos.Publishdatavariable;
 ...
 * @author user
 */
public interface PublishdatavariableRepositories extends MongoRepository<Publishdatavariable, String> {
     ...
     */
   @Query("{'flowId' : ?0}")
    public List <Publishdatavariable> getMxByFlowId(
   ...
   @Query("{'flowId' : ?0, 'vertexId' : ?1}")
    public Publishdatavariable getMxByFlowIdAndVertexId(
            @Param("flowId") String flowId,
    ...
    public List <Publishdatavariable> deleteDataVariableByFlowId(String flowId);
   ...
   @Query("{'flowId' : ?0, 'mxId' : ?1}")
    public Publishdatavariable findByMxId(
            @Param("flowId") String flowId,
   ...
   @Query("{'flowId' : ?0, $or: [ {'mxCell.@source':?1},{'mxCell.@target':?1}]}")
    public List<Publishdatavariable> findByUserObjectMxId(
            @Param("flowId") String flowId,
