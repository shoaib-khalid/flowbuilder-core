...
import com.kalsym.chatbot.flowbuilder.mxmodel.daos.Publishconnectionstart;
 ...
 * @author user
 */
public interface PublishconnectionstartRepositories extends MongoRepository<Publishconnectionstart, String> {
     ...
     */
   @Query("{'flowId' : ?0}")
    public List <Publishconnectionstart> getMxByFlowId(
    ...
    public List <Publishconnectionstart> deleteConnectionStartByFlowId(String flowId);
   ...
   @Query("{'flowId' : ?0, 'mxId' : ?1}")
    public Publishconnectionstart findByMxId(
            @Param("flowId") String flowId,
   ...
   public List<Publishconnectionstart> findByUserObjectMxId(
            @Param("flowId") String flowId,
